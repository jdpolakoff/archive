...
$(document).ready(function(){
  ...
  $('.dropDown').change(function(){
    ...
    var shows
    var identifiers
    var url = `https://archive.org/advancedsearch.php?q=Muddy+Waters&fl%5B%5D=identifier&sort%5B%5D=&sort%5B%5D=&sort%5B%5D=&rows=200&page=1&output=json&callback=callback&save=yes#raw`
    $.ajax({
      headers: {
    ...
    // }).then((response) => {
      ...
      // var identifiers1 = response.response.docs.filter(function(item){
      //   return item.format === 'VBR MP3'
      // })
      // var identifiers = identifiers1.map(function(item){
